(() => {
  ...
  const phase = latest?.label ? String(latest.label).replaceAll("_", " ") : "not recorded";
  ...
  document.getElementById("app").innerHTML = `<main class="shell" id="main">
    ...
    <div class="grid">
      ...
      <header class="chapter" id="decisions"><span>03</span><div><p class="eyebrow">Engineering view</p><h2>Decisions and boundaries</h2><p>Move from durable intent to the constraints, evidence, and journals that shaped implementation.</p></div></header>
      <section class="panel wide atlas-panel"><div class="panel-head"><div><h2>Decision Atlas</h2><p class="panel-copy">Start with the system, then open one decision surface at a time.</p></div><div class="atlas-tabs" id="atlas-tabs" role="tablist" aria-label="Decision Atlas views"><button class="active" data-atlas="overview" role="tab" aria-selected="true">Overview</button><button data-atlas="goals" role="tab" aria-selected="false">Goals</button><button data-atlas="constraints" role="tab" aria-selected="false">Constraints</button><button data-atlas="evidence" role="tab" aria-selected="false">Evidence</button><button data-atlas="journals" role="tab" aria-selected="false">Journals</button></div></div><div class="atlas-shell"><div id="decision-graph" role="img" aria-label="Focused knowledge relationship graph"></div><aside class="atlas-detail" aria-live="polite"><p class="eyebrow" id="atlas-kind">Overview</p><h3 id="atlas-title">Durable knowledge system</h3><p id="atlas-copy">Choose a view or select a card to inspect its full meaning. The canvas deliberately shows only one layer at a time.</p><button class="atlas-open" id="atlas-open" type="button" hidden>Open in Knowledge Explorer</button><nav class="atlas-items" id="atlas-items" aria-label="Visible Atlas items"></nav></aside></div></section>
  ...
  let atlasGraph;
  const atlasDetail = item => {
    const heading=item ? [item.kind,item.label,item.full] : atlasHeadings[$("#atlas-tabs button.active").dataset.atlas];
    $("#atlas-kind").textContent=heading[0]; $("#atlas-title").textContent=heading[1]; $("#atlas-copy").textContent=heading[2]||"No additional context recorded.";
    const action=item?.docId||item?.targetMode; $("#atlas-open").hidden=!action; $("#atlas-open").dataset.docId=item?.docId||""; $("#atlas-open").dataset.targetMode=item?.targetMode||""; $("#atlas-open").textContent=item?.targetMode?`Explore ${item.targetMode}`:"Open in Knowledge Explorer";
  };
  const drawAtlas = mode => {
    const all=atlasSource(mode), visible=all.slice(0,10), hidden=all.length-visible.length;
    const hub={id:"atlas-hub",label:mode==="overview"?"ClineFlow":"Focus",full:atlasHeadings[mode][2],kind:"hub",source:mode};
    const width=Math.max($("#decision-graph").clientWidth,720), height=Math.max($("#decision-graph").clientHeight,580);
    const placed=visible.map((item,index)=>{ if(mode==="overview"){const angle=(-Math.PI/2)+(index/visible.length)*Math.PI*2; return {...item,position:{x:width/2+Math.cos(angle)*width*.31,y:height/2+Math.sin(angle)*height*.32}};} const column=index%2,row=Math.floor(index/2),rows=Math.ceil(visible.length/2); return {...item,position:{x:width*(column?0.78:0.49),y:70+row*((height-140)/Math.max(rows-1,1))}}; });
    if(atlasGraph) atlasGraph.destroy();
    const elements=[{data:hub,position:{x:mode==="overview"?width/2:width*.16,y:height/2}},...placed.map(item=>({data:item,position:item.position})),...placed.map(item=>({data:{id:`edge-${item.id}`,source:"atlas-hub",target:item.id}}))];
    atlasGraph=cytoscape({container:$("#decision-graph"),elements,layout:{name:"preset",fit:true,padding:44},minZoom:.55,maxZoom:1.8,wheelSensitivity:.16,style:[{selector:"node",style:{"shape":"round-rectangle","width":178,"height":52,"background-color":"#102a27","border-color":"#3a756a","border-width":1,"label":"data(label)","color":"#dff9f1","font-family":"Space Grotesk","font-size":11,"font-weight":500,"text-wrap":"wrap","text-max-width":150,"text-valign":"center","text-halign":"center","overlay-opacity":0}},{selector:'node[kind="hub"]',style:{"width":126,"height":126,"shape":"ellipse","background-color":"#65fbd2","border-color":"#caffef","border-width":2,"color":"#04130f","font-size":14,"font-weight":700}},{selector:'node[kind="ledger"]',style:{"background-color":"#173a34","border-color":"#65fbd2","border-width":2}},{selector:'node[kind="active"]',style:{"border-color":"#69d9ff"}},{selector:'node[kind="complete"]',style:{"border-color":"#55756d","color":"#9dbdb4"}},{selector:'node[kind="constraint"]',style:{"border-color":"#ffc96b"}},{selector:'node[kind="evidence"]',style:{"border-color":"#b29cff"}},{selector:'node[kind="open"]',style:{"border-color":"#ff7f9f"}},{selector:'node[kind="journal"]',style:{"border-color":"#69d9ff"}},{selector:"node:selected",style:{"background-color":"#214b43","border-color":"#ffffff","border-width":2}},{selector:"edge",style:{"width":1,"line-color":"#28534a","target-arrow-color":"#3a756a","target-arrow-shape":"triangle","curve-style":"bezier","opacity":.64}}]});
    atlasGraph.on("tap","node",event=>atlasDetail(event.target.data()));
    $("#atlas-items").innerHTML=placed.map(item=>`<button type="button" data-atlas-id="${esc(item.id)}"><span>${esc(item.label)}</span><small>${esc(item.kind)}</small></button>`).join("")+(hidden?`<p class="atlas-more">+ ${hidden} more hidden to preserve legibility</p>`:"");
    $("#atlas-items").querySelectorAll("button").forEach(button=>button.addEventListener("click",()=>{const node=atlasGraph.$id(button.dataset.atlasId);atlasGraph.$("node").unselect();node.select();atlasGraph.animate({center:{eles:node},zoom:1.15},{duration:220});atlasDetail(node.data());}));
    atlasDetail();
  };
  const activateAtlas = mode => { $("#atlas-tabs").querySelectorAll("button").forEach(item=>{const active=item.dataset.atlas===mode;item.classList.toggle("active",active);item.setAttribute("aria-selected",String(active));});drawAtlas(mode); };
  $("#atlas-tabs").querySelectorAll("button").forEach(button=>button.addEventListener("click",()=>activateAtlas(button.dataset.atlas)));
